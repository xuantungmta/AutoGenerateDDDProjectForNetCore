    ...
    public partial class frmSelectEntity : Form
    {
        ...
        private void TvDependency_AfterCheck(object? sender, TreeViewEventArgs e)
        {
            if (e.Node != null)
            {
                ...
                if (e.Node.Checked)
                {
                    if (entities[index].Dependency.FindIndex(x => x.Entity == e.Node.Text) < 0)
                        entities[index].Dependency.Add(new EntityModel { Entity = e.Node.Text });
                }
                else entities[index].Dependency.RemoveAt(entities[index].Dependency.FindIndex(x => x.Entity == e.Node.Text));
            }
        }
        ...
        private void frmSelectEntity_Load(object sender, EventArgs e)
        {
            foreach (string table in tables)
            {
                if (!table.Contains('_'))
                {
                    ...
                    {
                        Entity = table,
                        Dependency = tables.Where(x => x != table).Where(x => x.StartsWith(table)).Select(x => new EntityModel { Entity = x }).ToList(),
                        IsChecked = true
                    };
                    entities.Add(entity);
                }
