...
namespace AutoGenerateDDDProjectForNetCore.helpers
{
    internal class Convention
    {
        ...
        internal const string DATBASE_USERNAME = "[$sql_username]";
        internal const string DATBASE_PASSWORD = "[$sql_password]";
    ...
    public static class GenereateHelper
    {
        ...
        public static void GenerateApiProject(string path, string projectName, EntityModel[] listEntity)
        {
            string pathApiProject = Path.Combine(path, $"{projectName}.api");
            // Generate entities
            Logging("Reading template file for api controller...");
            string contentController = File.ReadAllText("templates/entity/ApiController.tmp").Replace(Convention.PROJECT_NAME, projectName);

            string pathResponsitory = Path.Combine(pathApiProject, "Controllers");
            foreach (EntityModel entityModel in listEntity)
            {
                string content = contentController.
                  Replace(Convention.TABLE_NAME, entityModel.Entity);
                File.WriteAllText(Path.Combine(pathResponsitory, $"{entityModel.Entity}Controller.cs"), content);
            }
        }
    }
}
